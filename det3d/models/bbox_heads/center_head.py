...
class CenterHead(nn.Module):
    ...
    def loss(self, example, preds_dicts, **kwargs):
        ...
        for task_id, preds_dict in enumerate(preds_dicts):
            ...
            if self.semi_thresh > 0. and self.semi_weight > 0.:
                hm_loss = self.crit(preds_dict['hm'], example['hm'][task_id], example['ind'][task_id], example['mask'][task_id], example['cat'][task_id], example['det_scores'], self.semi_weight, self.semi_thresh, semi=True)
            else:
            ...
            if self.dataset in ['waymo', 'nuscenes', 'h3d']:
            ...
            if self.semi_thresh > 0. and self.semi_weight > 0.:      
                box_loss = self.crit_reg(preds_dict['anno_box'], example['mask'][task_id], example['ind'][task_id], target_box, example['det_scores'][task_id], semi=True)
            else:
